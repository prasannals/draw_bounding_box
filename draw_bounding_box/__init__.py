...
def draw_label_and_box(img, label, box, font_args):
    cv2.rectangle(img, tuple(box[:2]), tuple(box[2:]), 
        font_args['box_color'], font_args['box_thickness'] )

    cv2.putText(img, label, tuple(box[:2]), 
...
class DrawBoundingBox():
    def __init__(self, get_box_info, get_image, window_title='Bounding Box', quit_key='q',
        font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=0.5, font_color=(0,0,0), line_type=2,
        box_thickness=2, box_color=(0,255,0)):
        '''
        get_box_info - function - should return a list of tuples where the 
        first element of the tuple is the label of the box and the second 
        ...
        '''
        self.get_box_info, self.get_image = get_box_info, get_image
